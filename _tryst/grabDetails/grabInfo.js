const grabInfo = async (page, profileLink) => {
  ...
  try {
    ...
    let countryCodeElm = await countrySelector.evaluate((el) => el.textContent);
    let countryCodeArr = countryCodeElm.split(",");
    let countryCode = countryCodeArr[countryCodeArr.length - 1].trim();
    country = "";
  ...
  try {
    contactDataArr = await page.evaluate(() =>
      Array.from(
        ...
        [item.children[0].innerText.trim()]: item.children[1].innerText.trim(),
      }))
    );
  } catch (error) {
    console.log(error);
