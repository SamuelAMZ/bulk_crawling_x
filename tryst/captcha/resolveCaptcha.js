...
const resolveCaptcha = async (page, i) => {
  await page.waitForTimeout(5000);
  // get captcha svg link
  //   const elementHandle = await page.waitForSelector(".fancybox__content iframe");
  //   const frame = await elementHandle.contentFrame();
  //   const content = await frame.waitForSelector("img");
  ...
  page.on("response", async (response) => {
    ...
    if (request.url().includes("svg+xml")) {
      ...
      await getCaptchaNums(text, page);
    }
  });
  ...
  try {
    ...
    const button = await page.$("[title='Close']");
